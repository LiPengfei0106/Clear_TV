...
public class PlaybackOverlayFragment extends android.support.v17.leanback.app.PlaybackOverlayFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        tagKey = getActivity()
                .getIntent().getIntExtra(DetailsActivity.TAGKEY, 1);
        List<Movie> movies = MovieManager.movieLists.get(tagKey).movies;
            ...
            public void onItemClicked(Presenter.ViewHolder itemViewHolder, Object item,
                ...
                if (item instanceof Movie) {
                    if (mPlayPauseAction.getIndex() == PlayPauseAction.PLAY) {
                        mCallback.onFragmentPlayPause((Movie) item, 0, false);
                    } else {
                        mCallback.onFragmentPlayPause((Movie) item, 0, true);
                    }
                    mCurrentItem = mItems.indexOf(item);
...
//        playbackControlsRowPresenter.setBackgroundColor(0x88888888);
...
//        addOtherRows();
    ...
    public void back() {
        int position = mPlaybackControlsRow.getCurrentTime() - mPlaybackControlsRow.getTotalTime() / 100;
        if (position < 0)
            position = 0;
        mCallback.seekTo(position);
        mPlaybackControlsRow.setCurrentTimeLong(position);
    }

    public void fast() {
        int position = mPlaybackControlsRow.getCurrentTime() + mPlaybackControlsRow.getTotalTime() / 100;
        if (position > mPlaybackControlsRow.getTotalTime())
            position = mPlaybackControlsRow.getTotalTime();
        mCallback.seekTo(position);
        mPlaybackControlsRow.setCurrentTimeLong(position);
    }
...
//        mPlaybackControlsRow.setSecondaryActionsAdapter(mSecondaryActionsAdapter);
        ...
        mFastForwardAction = new PlaybackControlsRow.FastForwardAction(getActivity(), 5);
        mRewindAction = new PlaybackControlsRow.RewindAction(getActivity(), 5);

        mPrimaryActionsAdapter.add(mSkipPreviousAction);
//        mPrimaryActionsAdapter.add(mRewindAction);
//        mPrimaryActionsAdapter.add(mPlayPauseAction);
//        mPrimaryActionsAdapter.add(mFastForwardAction);
        mPrimaryActionsAdapter.add(new MyAction((int)mRewindAction.getId(), getActivity().getResources().getDrawable(R.drawable.lb_ic_fast_rewind)));
        mPrimaryActionsAdapter.add(mPlayPauseAction);
        mPrimaryActionsAdapter.add(new MyAction((int)mFastForwardAction.getId(), getActivity().getResources().getDrawable(R.drawable.lb_ic_fast_forward)));
...
//            item = mItems.get(mCurrentItem);
            item.setName(mItems.get(mCurrentItem).getName());
            item.setActor(mItems.get(mCurrentItem).getActor());
        }
        if (SHOW_IMAGE) {
    ...
    private void addOtherRows() {
        ...
        HeaderItem header = new HeaderItem(0, MovieManager.movieLists.get(tagKey).getCategoryNameByLanguageCode(Variables.language_code));
...
//    }
    ...
    public static class MyAction extends PlaybackControlsRow.MultiAction {
        ...
        public MyAction(int id, Drawable... drawables) {
            super(id);
